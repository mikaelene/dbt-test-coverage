    ...
    for source in schema["sources"]:
        ...
        for table in source['tables']:
            ...
            if "'tests':" in str(table):
                table_test = True
            test_status.append([source_name, source_db, source_schema, table_identifier, model_name, table_test])
...
def parse_schema(schema):
    ...
    try:
        ...
        if source_results:
            for source_result in source_results:
                print(f"Source: {source_result[0]}, Database: {source_result[1]}, Schema: {source_result[2]}, "
                      f"Table: {source_result[3]}, Model name: {source_result[4]}, Tested: {source_result[5]}" )
            print(" ")
            print(f"Sources: {source_results_agg[0]}, Tested: {str(source_results_agg[1])}, "
    ...
    try:
        ...
        if model_results:
            ...
            print(" ")
            print(f"Models: {model_results_agg[0]}, Tested: {str(model_results_agg[1])}, "
